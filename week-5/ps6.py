...
def get_story_string():
    ...
    """
    f = open("C:/Users/djgama/Dropbox/Online_courses/Edx-intro_to_cs_w_python_MIT6001x/week-5/problem_set/story.txt", "r")
    story = str(f.read())
    f.close()
    return story
...
WORDLIST_FILENAME = "C:/Users/djgama/Dropbox/Online_courses/Edx-intro_to_cs_w_python_MIT6001x/week-5/problem_set/words.txt"
...
class CiphertextMessage(Message):
    ...
    def decrypt_message(self):
        ...
        for x in range(26):
            wc = 0
            self.build_shift_dict(x)
            self.apply_shift(x)
            for word in self.apply_shift(x).split(' '):
                if  word in self.valid_words:
                    wc += 1
            if wc > count:
                count = wc
                b_shift = x
        return (b_shift, self.apply_shift(b_shift))
